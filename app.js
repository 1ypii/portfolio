(function () {
  ...
  function buildHero() {
    ...
    if (socials.length) {
      ...
      socials.forEach(function (s) {
        ...
        if (String(s.label).toLowerCase() === "roblox") a.hidden = true;
  ...
  function buildCell(src, caption, label) {
    ...
    v.addEventListener("error", function () {
      cell.innerHTML = "<span class='cell-missing'>refresh to see ts</span>";
    });
  ...
  function buildCardMedia(thumb, pillText, titleHtml) {
    ...
    media.appendChild(el("div", "card-shade"));
    if (pillText) media.appendChild(el("span", "card-pill", pillText));
    media.appendChild(el("div", "card-title", titleHtml));
    return media;
  }
  ...
  function buildGameCard(p, g) {
    ...
    var pill = (g.playing > 0)
      ? compact(g.playing) + " playing"
      : (g.visits != null ? compact(g.visits) + " visits" : "");
    card.appendChild(buildCardMedia(g.thumb, pill, esc(p.title || g.name)));

    var body = el("div", "card-body");
    if (g.creator && g.creator.name) {
      body.appendChild(el("div", "card-by",
    ...
    if (g.desc) body.appendChild(el("p", "card-desc", esc(g.desc)));
    if (p.description) body.appendChild(el("p", "card-note", esc(p.description)));
    if (g.proof) {
      var pf = el("span", "proof-link", "view proof ↗");
      pf.addEventListener("click", function (e) {
        e.preventDefault();
        window.open(g.proof, "_blank", "noopener");
      });
      body.appendChild(pf);
    }
  ...
  function buildGroupCard(p, g) {
    ...
    var pill = g.members ? compact(g.members) + " members" : "";
    var title = esc(p.title || g.name || "group") + (g.verified ? " " + VERIFIED_BADGE : "");
    card.appendChild(buildCardMedia(g.thumb, pill, title));
  ...
  function buildWorkSection(group) {
    ...
    var sec = section(group.category || "work");
    ...
    if (projects.length) {
      ...
      var allGames = projects.every(function (p) { return placeIdFromUrl(p.url); });
      var list = el("div", (allGroups || allGames) ? "cards" : "rows");
  ...
  function buildUpdates() {
    var items = (cfg.updates || []).filter(Boolean);
    if (!items.length) return null;

    var sec = section("updates");
    var list = el("div", "notes");
    items.forEach(function (u) {
      var note = el("article", "note");
      if (u.date) note.appendChild(el("div", "note-date", esc(u.date)));
      note.appendChild(el("p", "note-text", esc(u.text || "")));
      if (u.link && u.link.url) {
        var a = el("a", "proof-link", esc(u.link.label || u.link.url) + " ↗");
        a.href = u.link.url;
        if (external(u.link.url)) { a.target = "_blank"; a.rel = "noopener noreferrer"; }
        note.appendChild(a);
      }
  ...
  function buildNav() {
    ...
    if ("IntersectionObserver" in window) {
      var mark = el("div", "secnav-mark");
      nav.parentNode.insertBefore(mark, nav);
      new IntersectionObserver(function (entries) {
        nav.classList.toggle("is-stuck", !entries[0].isIntersecting);
      }, { rootMargin: "-11px 0px 0px 0px" }).observe(mark);
